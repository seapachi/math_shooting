...
### タイトル欠け再発の再調査と修正（横幅超過対策）
前回は「フォント読み込みタイミング」を主因として対策しましたが、再発画面を確認すると、
`MATH LANE SHOOTER` 自体の描画幅が390px基準画面に対して大きく、左右がキャンバス外へ出てクリップされるケースがあることが分かりました。

今回、`StartScene` のタイトル生成直後に幅チェックを追加し、次の条件で自動縮小するようにしています。

- 目標幅: `画面幅 - 20px`（左右に最小余白を確保）
- 初期サイズ: `36px`
- 最小サイズ: `24px`
- `title.width` が目標幅を超える間、`1px` ずつ縮小

これにより、フォントや描画環境差で字幅が広くなった場合でも、タイトルが画面内に収まり、
iPhone Chromeでの左右欠けを起こしにくくしています。
