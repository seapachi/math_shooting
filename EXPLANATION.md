...
### LIFE表示の数値ラベル削除（バーのみ表示）
LIFEパネルは、数値ラベル（`LIF: n`）を表示せず、3本のバーだけで残機を示す仕様に変更しました。

- 残機表示はバーの点灯数のみで表現
- 残機が減ると、対応するバーを暗色に切り替え
- 被弾/見逃し時のフィードバックはLIFEテキストではなくバー全体にパルス演出を適用

これにより、横幅の小さい端末でもLIFEパネル内の文字はみ出しを避けつつ、残機情報を視認しやすくしています。
