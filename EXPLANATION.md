...
- 不正解ヒットや見逃しでライフ減少
- 正解したラウンドのたびに落下スピードが1段階上昇（不正解時は速度維持）
- ライフが0になると終了
...
### 自機テクスチャ（`player_sharp` / `player_flare`）の視認性改善
`BootScene.create()` の自機生成ロジックを、レイヤーを分けた描画順に整理しました。

- 描画レイヤーを **翼 → 主胴体 → コックピット → エンジン** の順に固定
- 配色を共通化
  - 縁取り: シアン
  - 機体本体: 暗色ネイビー
  - 噴射: オレンジ
- 翼端ライト（小円）を左右に追加
- エンジン後方に噴射粒子（小円）を追加
- `generateTexture()` のサイズは既存と同じ
  - `player_sharp`: `88 x 56`
  - `player_flare`: `88 x 62`

これにより、プレイヤー当たり判定の中心となる `this.player` の挙動を変えずに、画面上での視認性だけを向上しています。
