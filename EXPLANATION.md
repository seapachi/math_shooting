...
### フォントと選択肢パネルの見やすさ改善
`GameScene` の選択肢表示を、遠目でも読み取りやすい方向に調整しています。

- `BootScene.create()` の `choice` テクスチャを `78x44` から `110x76` 相当に拡大
- 選択肢パネルは2層構造
  - 外側: オレンジの外枠プレート
  - 内側: 暗色の半透明プレート
- 敵数字（選択肢）
  - フォント: `FONT_ENEMY`（Rajdhani）
  - サイズ: `50px`
  - 色: `#ff8800` に統一
  - グロー: オレンジ系の `shadow blur` を強めて発光感を追加
