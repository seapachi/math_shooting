...
- `index.html` の敵（選択肢）テクスチャを「塗りなし・枠線あり」に変更しました。数字表示仕様は維持しています。
- `index.html` の選択肢数字の色を明るくし、ストロークを追加して透明背景でも見えるように調整しました。
- `index.html` の敵（選択肢）数字フォントを `FONT_UI` から `FONT_TITLE`（Orbitron優先）に変更しました。
- `index.html` の敵（選択肢）をスコア色 `#fbbf24` に合わせ、枠線のみの弱いグロー表現に変更しました。
- `index.html` の敵（選択肢）数字を太字にして視認性を上げました。
